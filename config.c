...
#pragma config FCMENP = OFF     // Fail-Safe Clock Monitor -Primary XTAL Enable bit (FSCM timer will not set FSCMP bit or OSFIF interrupt on Primary XTAL failure)
#pragma config FCMENS = OFF     // Fail-Safe Clock Monitor -Secondary XTAL Enable bit (FSCM timer will not set FSCMS bit or OSFIF interrupt on Secondary XTAL failure)
